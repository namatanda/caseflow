...
CourtFlow Backend is a TypeScript-powered Express API that backs the CourtFlow performance dashboard. It delivers secure REST endpoints, orchestrates data via Prisma (SQLite for local development, PostgreSQL for production), and provides robust authentication/session management.
...
> If `.env.example` is absent, create `.env` manually using the variables listed in [Environment Configuration](#environment-configuration). Local development uses SQLite (zero-config), while Docker/production uses PostgreSQL.
